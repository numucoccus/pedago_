      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-6 rounded-xl border border-border bg-card p-6 space-y-4 shadow-xs">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-muted-foreground">Percentile Basis</span>
          </div>
        ...
        <div className="lg:col-span-6 rounded-xl border border-border bg-card p-6 space-y-4 shadow-xs flex flex-col justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="swiss-header-tag text-amber-600 dark:text-amber-400">
          ...
          <div className="space-y-3 pt-1">
            {student.workloadSignals.map((ws, i) => (
              <div
                key={i}
                className="p-3.5 rounded-lg border border-border/80 bg-background/60 space-y-1 text-xs"
              >
                <div className="flex items-center justify-between">
                  <span className="font-bold text-foreground">{ws.signal}</span>
                  <span className="text-[0.625rem] uppercase font-mono px-2 py-0.5 rounded bg-muted text-muted-foreground">
          ...
          <div className="p-3 rounded-lg bg-muted/30 border border-border/60 text-[0.6875rem] text-muted-foreground">
        ...
        <div className="space-y-3">
          {student.achievements.map((ach) => {
            ...
            return (
              <div
                key={ach.id}
                className="p-4 rounded-xl border border-border/80 bg-background/60 space-y-2 text-xs"
              >
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                  ...
                  <span
                    className={cn(
                      "text-[0.625rem] font-bold px-2.5 py-1 rounded-md border tracking-tight shrink-0",
                      badge.className
                    )}
                  >
                    {badge.label}
                ...
                {ach.evidenceLocator && (
                  <div className="text-[0.6875rem] text-primary bg-primary/5 p-2 rounded border border-primary/15 font-mono">
